...
namespace gnut
{
    ...
    double t_gsetproc::ion_height()
    {
        ...
        str_erase(tmp);
        double value = tmp.empty() ? 350000.0 : std::stod(tmp);  
        _gmutex.unlock();
        return value;
    }
    ...
    int t_gsetproc::mapping_function()
    {
        ...
        str_erase(tmp);
        int value = tmp.empty() ? 0 : std::stoi(tmp); 
        _gmutex.unlock();
        return value;
    }

    bool t_gsetproc::rec_dcb_corr()
    {
        _gmutex.lock();
        string tmp = _doc.child(XMLKEY_ROOT).child(XMLKEY_PROC).child_value("rec_dcb_corr");
        str_erase(tmp);
        transform(tmp.begin(), tmp.end(), tmp.begin(), ::toupper);
