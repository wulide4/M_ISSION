...
namespace gnut
{
    ...
    std::vector<double> t_gallobs::get_station_coordinates(const std::string& site) const {
        std::vector<double> coordinates(3, 0.0);  // Returns (0, 0, 0) if the station is not found


        auto it = _mapcrds.find(site);
        if (it != _mapcrds.end()) {
            coordinates[0] = it->second[0];
            coordinates[1] = it->second[1];
            coordinates[2] = it->second[2];
        }
