...
int great::t_gpvtflt::_processEpoch(const t_gtime &runEpoch)
{
    ...
    std::map<std::string, std::string> prn2code_this_epoch;
    std::map<std::string, std::string> prn2phase_this_epoch;   
    ...
    for (const auto& iter : _data) {
        ...
        std::string code_used = gobs2str(iter.select_range(b1)) + "," + gobs2str(iter.select_range(b2));
        std::string phase_used = gobs2str(iter.select_phase(b1)) + "," + gobs2str(iter.select_phase(b2));

        prn2code_this_epoch[prn] = code_used;
        prn2phase_this_epoch[prn] = phase_used;
    }
    ...
    for (const auto& param : allParams) {
        if (param.parType == par_type::SION) {
            ...
            auto itp = prn2phase_this_epoch.find(param.prn);
            if (itp != prn2phase_this_epoch.end()) {
                phase_used = itp->second;
            }

            // eg: "C1W,C2W|L1W,L2W"
            _sion_memory.push_back(std::make_tuple(
                runEpoch, param.prn, param.parType, param.value(),
                code_used + "|" + phase_used
            ));
        }
    }
