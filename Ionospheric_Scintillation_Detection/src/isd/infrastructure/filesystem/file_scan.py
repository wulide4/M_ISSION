...
def _ecef_to_lla(x: float, y: float, z: float) -> tuple[float, float, float]:
    """Convert ECEF (X, Y, Z) in meters to geodetic (lat, lon, alt) in degrees/meters.
    ...
    nu = a / math.sqrt(1 - e2 * sin_lat * sin_lat)
    alt = p / cos_lat - nu if abs(cos_lat) > 1e-10 else abs(z) - b
    ...
    def _upsert_station(
        ...
        if station_code not in stations_by_code:
            ...
            if isinstance(approx_xyz, (list, tuple)) and len(approx_xyz) >= 3:
                try:
                    ex, ey, ez = float(approx_xyz[0]), float(approx_xyz[1]), float(approx_xyz[2])
                    latitude, longitude, height = _ecef_to_lla(ex, ey, ez)
