    ...
    def validate_task(self, payload: dict) -> ApiResponse[dict]:
        ...
        if MetricKey.SIGMA_PHI_F in config.metrics:
            ...
            if not provider_summary.get("allAntennaAvailable", False):
                issues.append(self._issue("SIGMAPHI_NEED_ATX", "sigma-phi-f requires ATX", True))
            ...
            if not orbit_formal_ready:
                if config.enable_nav_fallback and allow_nav_degraded_mode and orbit_available and has_nav:
                    ...
                    if not orbit_available:
                        issues.append(
                            self._issue(
                                "PROVIDER_ORBITCLOCK_UNAVAILABLE",
                                "Orbit/clock provider cannot satisfy selected dates",
                                True,
                            )
                        )
                    if sp3_missing:
                        issues.append(self._issue("SIGMAPHI_NEED_SP3", "sigma-phi-f requires SP3", True))
                    if clk_missing:
                        issues.append(self._issue("SIGMAPHI_NEED_CLK", "sigma-phi-f requires CLK", True))
