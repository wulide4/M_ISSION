...
from isd.ui.pages.preprocess_page import PreprocessPage
from isd.ui.widgets.side_nav import SideNav
from isd.ui.widgets.top_bar import TopBar


# Nav item keys → page index mapping
_NAV_KEYS = [
    ("nav.home", "home"),
    ("nav.task", "task"),
    ("nav.upload", "upload"),
    ("nav.preprocess", "preprocess"),
    ("nav.visual", "visual"),
...
class MainWindow(QMainWindow):
    def __init__(self, context: AppContext) -> None:
        ...
        self.preprocess_page = PreprocessPage(context.command_bus)
        ...
        # Add pages in the same order as _NAV_KEYS
        self._pages = {
            "home": self.home_page,
            "task": self.task_page,
            "upload": self.data_upload_page,
            "preprocess": self.preprocess_page,
            "visual": self.visual_page,
            "analysis": self.analysis_page,
            "report": self.report_page,
            "settings": self.settings_page,
        }

        for key, page_key in _NAV_KEYS:
            title = tr(key)
    ...
    def _retranslate(self) -> None:
        self.setWindowTitle(tr("app.title"))
        for i, (key, _) in enumerate(_NAV_KEYS):
    ...
    def _on_nav_changed(self, idx: int) -> None:
        if idx < 0 or idx >= len(_NAV_KEYS):
            return
        page_key = _NAV_KEYS[idx][1]
        cur_proj = self.data_upload_page.current_project_id

        if page_key == "visual" and cur_proj:
            self.visual_page.set_project(cur_proj)
        elif page_key == "preprocess" and cur_proj:
            self.preprocess_page.set_project(cur_proj)
            task_data = self.visual_page.task_selector.currentData()
            if task_data and task_data.get("project_id"):
                self.preprocess_page.project_id = task_data["project_id"]
                self.preprocess_page._load_task_list()
        elif page_key == "analysis" and cur_proj:
            self.analysis_page.set_project(cur_proj)
            self.analysis_page.refresh()
        elif page_key == "report" and cur_proj:
