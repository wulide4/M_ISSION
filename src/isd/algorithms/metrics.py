...
class ScintillationConfig:
    ...
    # S4C per Zhang et al. (2026) / IonoMoni calc_S4C
    s4c_n_trend: int = 60   # detrending window (epochs)
    s4c_l_stat: int = 60    # statistical window (epochs)
...
def dixsg_grid(
    ...
    # DIXSG requires at least 2 stations per Get_DIXSG.m (num_files < 2 -> continue)
    return grid
...
def _s4c_single(snr_col: np.ndarray, n_trend: int, l_stat: int) -> np.ndarray:
    """Compute S4C for a single satellite's SNR data.

    Uses the same sliding-window sum approach as IonoMoni calc_S4C
    (calc_ROTI.cpp lines 521-654) for exact numerical consistency.
    """
    n_rows = len(snr_col)
    s4c = np.full(n_rows, np.nan, dtype=float)

    # Step 1: dB-Hz -> linear power
    s_lin = np.full(n_rows, np.nan, dtype=float)
    with np.errstate(invalid="ignore"):
        valid = np.isfinite(snr_col) & (snr_col > 0)
        s_lin[valid] = np.power(10.0, 0.1 * snr_col[valid])

    # Step 2: Detrending via sliding-window running-mean normalization
    # x[k] = s_lin[k] / mean(s_lin[k-n_trend ... k-1])
    x = np.full(n_rows, np.nan, dtype=float)
    sum_prev = 0.0
    cnt_prev = 0

    for k in range(n_rows):
        # Add s_lin[k-1] into window
        if k - 1 >= 0 and np.isfinite(s_lin[k - 1]):
            sum_prev += s_lin[k - 1]
            cnt_prev += 1
        # Remove s_lin[k-n_trend-1] from window
        if k - n_trend - 1 >= 0 and np.isfinite(s_lin[k - n_trend - 1]):
            sum_prev -= s_lin[k - n_trend - 1]
            cnt_prev -= 1

        if cnt_prev == n_trend and np.isfinite(s_lin[k]) and sum_prev > 0.0:
            x[k] = s_lin[k] / (sum_prev / n_trend)

    # Step 3: S4C = sqrt(var) / mean over sliding window of l_stat on x
    sum_x = 0.0
    sum_xsq = 0.0
    cnt_x = 0
    ...
    for k in range(n_rows):
        ...
        if k - l_stat >= 0:
            if np.isfinite(x[k - l_stat]):
                sum_x -= x[k - l_stat]
                sum_xsq -= x[k - l_stat] * x[k - l_stat]
                cnt_x -= 1

        if k >= n_trend + l_stat - 1:
            if cnt_x == l_stat:
                m = sum_x / l_stat
                v = max(sum_xsq / l_stat - m * m, 0.0)
                s4c[k] = np.sqrt(v) / m if m > 0.0 else 0.0

    return s4c


def compute_s4c(
    snr_l1_dbhz: np.ndarray,
    snr_l2_dbhz: np.ndarray | None = None,
    n_trend: int = 60,
    l_stat: int = 60,
) -> tuple[np.ndarray, np.ndarray | None]:
    """SNR-based amplitude scintillation index S4C per Zhang et al. (2026).

    Matches IonoMoni calc_S4C (calc_ROTI.cpp lines 521-654) exactly:
      1. Convert SNR from dB-Hz to linear power: s = 10^(0.1 * S_dBHz)
      2. Detrend via running-mean normalization (sliding-window sum):
         x[k] = s[k] / mean(s[k-n_trend ... k-1])
      3. S4C = sigma_x / mu_x over sliding window of l_stat epochs
         using E[x^2] - (E[x])^2 for variance

    Computes S4C for both S1 and S2 frequencies (dual-frequency).

    Args:
        snr_l1_dbhz: (N,) or (N, C) SNR L1 array in dB-Hz. NaN for invalid epochs.
        snr_l2_dbhz: (N,) or (N, C) SNR L2 array in dB-Hz, or None.
        n_trend: detrending window length (default 60 epochs).
        l_stat: statistical window length (default 60 epochs).

    Returns:
        (s4c_s1, s4c_s2) — S4C arrays same shape as inputs.
        s4c_s2 is None when snr_l2_dbhz is None.
    """
    if snr_l1_dbhz.ndim == 1:
        snr_l1_dbhz = snr_l1_dbhz.reshape(-1, 1)
    if snr_l2_dbhz is not None and snr_l2_dbhz.ndim == 1:
        snr_l2_dbhz = snr_l2_dbhz.reshape(-1, 1)

    n_rows, n_cols = snr_l1_dbhz.shape
    s4c_s1 = np.full((n_rows, n_cols), np.nan, dtype=float)

    for col in range(n_cols):
        s4c_s1[:, col] = _s4c_single(snr_l1_dbhz[:, col], n_trend, l_stat)

    s4c_s2 = None
    if snr_l2_dbhz is not None:
        s4c_s2 = np.full(snr_l2_dbhz.shape, np.nan, dtype=float)
        for col in range(snr_l2_dbhz.shape[1]):
            s4c_s2[:, col] = _s4c_single(snr_l2_dbhz[:, col], n_trend, l_stat)

    return s4c_s1, s4c_s2
