...
class ResultService:
    ...
    def get_grid(self, payload: dict) -> ApiResponse[dict]:
        ...
        # Fallback: extract from mbl array [maxlon, minlon, maxlat, minlat, levels]
        if "lat_range" not in response_data and "mbl" in data:
            mbl = np.asarray(data["mbl"]).flatten()
            if len(mbl) >= 5:
                response_data["lon_range"] = [float(mbl[1]), float(mbl[0])]
                response_data["lat_range"] = [float(mbl[3]), float(mbl[2])]
