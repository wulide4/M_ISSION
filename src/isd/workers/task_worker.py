...
class NoDataForSystemError(Exception):
    """Raised when a RINEX file exists but contains no data for the requested GNSS system."""
...
class TaskWorkerManager:
            ...
            try:
                for sub in subtasks:
                    ...
                    for metric in task.config.metrics:
                        ...
                        except NoDataForSystemError as nodata:
                            self._step(
                                runtime,
                                task.id,
                                sub.id,
                                ProcessingStep(
                                    ...
                                    started_at=started_at,
                                    finished_at=utc_now(),
                                    output_summary=f'skipped_no_data:{sub.system.value}',
                                ),
                            )
                            self._log(
                                runtime,
                                ...
                                'WARN',
                                step_key,
                                f'Skipped {metric.value} for {sub.station_id} {sub.date}: {nodata}',
                            )
                            runtime.done += 1
    ...
    def _compute_metric(
        ...
        # DIXSG requires multi-station cROT bundles — use dedicated path
        if metric == MetricKey.DIXSG:
            return self._compute_dixsg_metric(runtime, task, sub)

        # Try to load real RINEX OBS data for this station
        rinex_result, scint_result, has_system_data = self._compute_from_rinex(runtime, task, sub, metric)
        if not has_system_data:
            raise NoDataForSystemError(
                f'{sub.station_id} has no {sub.system.value} observation data'
            )
        if rinex_result is not None:
            ...
            # Append IPP coordinates to the NPZ file for grid binning visualization.
            # AATR is hourly-aggregated (1D), shape doesn't match IPP (2D).
            # For AATR, save the raw per-epoch IAATR alongside IPP so the map
            # can still use IPP grid binning.
            if scint_result is not None and scint_result.ipp_b is not None and scint_result.ipp_l is not None:
                result_file = task_dir / 'results' / f'{sub.id}_{metric.value}.npz'
                if result_file.exists():
                    existing = dict(np.load(result_file))
                    existing['ipp_b'] = np.degrees(scint_result.ipp_b)
                    existing['ipp_l'] = np.degrees(scint_result.ipp_l)
                    if metric == MetricKey.AATR and scint_result.iaatr is not None:
                        existing['iaatr_raw'] = scint_result.iaatr
                    np.savez(result_file, **existing)
        ...
        # No RINEX data available for this metric — skip entirely
        raise NoDataForSystemError(
            f'No observation data available for {sub.station_id} {sub.system.value} {metric.value}'
        )

    def _compute_dixsg_metric(
        self,
        runtime: TaskRuntime,
        task: Task,
        sub: SubTask,
    ) -> tuple[str, ResultStats, str | None]:
        """Compute DIXSG from multi-station cROT bundles (MATLAB golden or Python chain)."""
        task_dir = runtime.task_dir
        assert task_dir is not None

        bundle, source = self._load_dixsg_bundle(runtime, task, sub)
        if bundle is None:
            raise NoDataForSystemError(
                f'DIXSG requires cROT from ≥2 stations for {sub.date} {sub.system.value} (source={source})'
            )

        # bundle.adixsg is 1-D (hourly) and bundle.ll is 3-D spatial grid (24×nlat×nlon)
        series = bundle.adixsg
        grid = bundle.ll
        dixsg_cfg = task.config.algorithm_config.dixsg

        data_path = self.result_store.save_payload(
            task_dir / 'results' / f'{sub.id}_DIXSG.npz',
            {
                'time': np.arange(series.shape[0], dtype=int),
                'values': series,
                'grid': grid,
                'lat_range': [float(dixsg_cfg.lat_range[0]), float(dixsg_cfg.lat_range[1])],
                'lon_range': [float(dixsg_cfg.lon_range[0]), float(dixsg_cfg.lon_range[1])],
            },
        )

        artifact_path = None
        if task.config.enable_intermediate_save:
            artifact = task_dir / 'intermediate' / f'{sub.id}_DIXSG.json'
            artifact.write_text(
                json.dumps(
                    {
                        'source': source,
    ...
    def _trace_sigmaphi_pipeline(
        ...
        # Polynomial detrending (degree=3) is applied INSIDE butterworth_filter()
        # per butterworthband.m line 52: detrend(segment, 3).
    ...
    def _compute_from_rinex(
        ...
        sub: SubTask,
        metric: MetricKey,
    ) -> tuple[np.ndarray | None, object | None, bool]:
        """Try to compute a metric from real RINEX OBS data.

        Returns (result_array_or_None, scintillation_result_or_None, file_has_system_data).
        file_has_system_data is False when the RINEX file exists but contains
        no observation data for the requested GNSS system (e.g. ALBH has no BDS).
        """
        from isd.algorithms.metrics import ScintillationConfig

        obs_path = self._find_rinex_obs_path(sub.station_id, sub.date)
        if not obs_path:
            return None, None, True  # no file at all — not a system-mismatch issue

        try:
            use_precise, clk_paths, atx_path = self._check_precise_products(task, sub.date)
            ...
            # RINEX file exists but has no data for this system
            if result.num_satellites == 0:
                return None, None, False

            if metric == MetricKey.ROTI and result.roti is not None:
                arr = result.roti
                return (arr if np.isfinite(arr).sum() > 10 else None), result, True
            if metric == MetricKey.IAATR and result.iaatr is not None:
                arr = result.iaatr
                return (arr if np.isfinite(arr).sum() > 10 else None), result, True
            if metric == MetricKey.AATR and result.aatr is not None:
                arr = result.aatr
                return (arr if np.isfinite(arr).sum() > 5 else None), result, True
            if metric == MetricKey.SIGMA_PHI_F and result.sigma_phi_f is not None:
                arr = result.sigma_phi_f
                return (arr if np.isfinite(arr).sum() > 10 else None), result, True
            if metric == MetricKey.S4C and result.s4c is not None:
                arr = result.s4c
                return (arr if np.isfinite(arr).sum() > 0 else None), result, True
    ...
    def _check_precise_products(
        self, task: Task, date_text: str,
    ) -> tuple[bool, list[str] | None, str | None]:
        """Check if SP3+CLK+ATX precise products are available from provider_metadata."""
        provider_meta = task.config.provider_metadata or {}
        date_providers = provider_meta.get('dateProviders') or {}
        date_meta = date_providers.get(date_text) or {}

        sp3_paths = date_meta.get('sp3Paths')
        has_sp3 = bool(sp3_paths and any(Path(p).exists() for p in sp3_paths))

        clk_paths = date_meta.get('clkPaths')
        has_clk = bool(clk_paths and any(Path(p).exists() for p in clk_paths))

        atx_path = date_meta.get('atxPath') or provider_meta.get('atxPath')
        has_atx = bool(atx_path and Path(atx_path).exists())

        return (has_sp3 and has_clk and has_atx), clk_paths, atx_path
