...
%% GPT2 (Global Pressure and Temperature 2) Model
% Function:
%     Provides key meteorological parameters for a given location and time based
%     on the GPT2 model. It reads data from the 'gpt2_5.grd' grid file and
%     interpolates values for pressure, temperature, lapse rate, water vapor
%     pressure, mapping function coefficients, and geoid undulation.
%
% INPUT:
%     dmjd:  Modified Julian Date (MJD) of the target epoch.
%     dlat:  Geodetic latitude of the station .
%     dlon:  Geodetic longitude of the station .
%     hell:  Ellipsoidal height of the station .
%     nstat: Number of stations.
%     it:    Flag for time variability (1: for mean values, otherwise: for
%            values with annual and semi-annual variations).
%
% OUTPUT:
%     p:     Pressure at the station's location .
%     T:     Temperature at the station's location.
%     dT:    Temperature lapse rate.
%     e:     Water vapor pressure.
%     ah:    Hydrostatic mapping function coefficient 'a' .
%     aw:    Wet mapping function coefficient 'a' .
