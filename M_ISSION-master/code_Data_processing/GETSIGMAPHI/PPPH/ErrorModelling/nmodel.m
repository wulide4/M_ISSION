...
%% GNSS Observation Modeling
% Function:
%     Computes various error correction models required for high-precision GNSS
%     data processing. It accounts for a wide range of physical effects and
%     systematic errors to generate a detailed correction model for each raw
%     observation (P1/P2/L1/L2). Calculated corrections include, but are not
%     limited to: satellite clock bias, relativistic effects, satellite and
%     receiver antenna phase center corrections, tropospheric delay, phase
%     wind-up, and solid earth tides.
%
%     The user can flexibly enable or disable specific error models via the
%     `options` parameter.
%
% INPUT:
%     data:       A structure containing GNSS observation data, satellite ephemeris,
%                 antenna information (ATX file data), etc.
%     options:    A structure that controls the modeling options, specifying which
%                 error terms should be computed.
%
% OUTPUT:
%     model:      A matrix that provides a detailed record of each error correction
%                 component and the total correction (based on `options`) for every
%                 observation. 
%% ---------------------------------------------------------------------
...
for i=1:size(data.obs.st,1)
    ...
    for k = sats
        ...
        for u=1:4
            ...
            full = model(t,15:25);
            model(t,7) = sum(full(~isnan(full(mopt==1))));
        end
    end
end
end
