function [Sites_Info]=R_rinex(r_ipath,r_opath)
%% Four system observation data
% INPUT:
        ...
        if length(line)>78 && strcmp('C',line(1)) && strcmp('SYS / # / OBS TYPES',line(61:79))
            ...
            for i=1:obsbds_n
                if strcmp(bdst(1,i),'L2I') %L2I -> L2
                    bdsloc(1)=i;
                elseif strcmp(bdst(1,i),'L1I') %L1I -> L1
                    bdsloc1(1)=i;
                    continue;
                end

                if strcmp(bdst(1,i),'L6I') %L6I -> L6
                    bdsloc(2)=i;
                elseif strcmp(bdst(1,i),'L7I')&& any(strcmp(bdst,'L1I'))
                    bdsloc1(2)=i;
                    continue;
                end
                if strcmp(bdst(1,i),'C2I') %C2I -> C2
                    bdsloc(3) = i;
                elseif strcmp(bdst(1,i),'C1I') %C1I -> C1
                    bdsloc1(3)=i;
                    continue;
                end

                if strcmp(bdst(1,i), 'C6I')%C6I -> C6
                    bdsloc(4)=i;
                elseif strcmp(bdst(1,i),'C7I')&& any(strcmp(bdst,'C1I'))
                    bdsloc1(4)=i;
                    continue;
                end
            end
            if exist('bdsloc','var')
                bdsloc(1,length(bdsloc)+1:4)=0;
            ...
            while 1
                ...
                if bdsmark~=0 && exist('bdsloc','var')
                    if length(line)>max(bdsloc)*16 && strcmp(line(1),'C')
                        ...
                        obs.BDSL6I(ep,sv_C)=str2double(line(16*bdsloc(2)-12:16*bdsloc(2)+1));
                        obs.BDSC6I(ep,sv_C)=str2double(line(16*bdsloc(4)-12:16*bdsloc(4)+1));
                    end
                end
                if bds1mark~=0 && exist('bdsloc1','var')
                    if length(line)>max(bdsloc1)*16 && strcmp(line(1),'C')
                        sv_C=str2double(line(2:3));
                        obs.BDSL1I(ep,sv_C)=str2double(line(16*bdsloc1(1)-12:16*bdsloc1(1)+1));
                        obs.BDSC1I(ep,sv_C)=str2double(line(16*bdsloc1(3)-12:16*bdsloc1(3)+1));
                        obs.BDSL7I(ep,sv_C)=str2double(line(16*bdsloc1(2)-12:16*bdsloc1(2)+1));
                        obs.BDSC7I(ep,sv_C)=str2double(line(16*bdsloc1(4)-12:16*bdsloc1(4)+1));
                    end
                end
            end
        end
